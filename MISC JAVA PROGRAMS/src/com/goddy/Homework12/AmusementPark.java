...
public class AmusementPark {
    ...
    public static void main(String[] args) {
        ...
        JOptionPane.showMessageDialog(null,
                "\t=========Welcome! Select your Amusement Park=========\n" +
                        "\n\nEnjoy!\n\n\n------------------------------------------------------\n" +
                        "Note: Multi-park Selections will cost an additional 30% up-charge" +
                        "\n------------------------------------------------------"
                , "Amusement Park",
                JOptionPane.INFORMATION_MESSAGE);
        String[] choices = {"Walt Disney World", "Universal Studios", "Beto Carrero World", "Cedar Point"};
        String decision = (String) JOptionPane.showInputDialog(null, "Choose Amusement Park",
                "Amusement Park",
                JOptionPane.QUESTION_MESSAGE, null, choices, choices[0]);
        if (decision == null) {
            return;
        }
        int[] priceslist = priceList(decision);
        int currentprice = 0;
        String message = null;
        do {
            Object[] options = {"Add New Ticket", "Check Order", "Checkout"};
            int dec = JOptionPane.showOptionDialog(null, "Main Menu", "Amusement Park: "
                            + decision, JOptionPane.YES_NO_CANCEL_OPTION,
            ...
            if (dec == 0) {
                ...
                if (park.equals("Cancel")) {
                    ...
                    if (classype.equals("Cancel")) {
                        ...
                        switch (classype) {
                            case "Child":
                                currentprice = price(priceslist, 0);
                                break;
                            case "Student":
                                currentprice = price(priceslist, 1);
                                break;
                            case "Adult":
                                currentprice = price(priceslist, 2);
                                break;
                            case "Senior":
                                currentprice = price(priceslist, 3);
                        }
                        userSelection.add(park + " " + classype + " " + String.valueOf(currentprice));
                        message = "lol";
                        continue;
                    }
                }
                ...
                if (userSelection.size() == 0) {
                    message = option8Warning();
                    continue;
                } else {
                    double price = priceCalc(userSelection) * 1.07; // 7 percent tax
                    int checkout = JOptionPane.showConfirmDialog(null, "Total Price: $" +
    ...
    public static String option4() {
        ...
        String decision = (String) JOptionPane.showInputDialog(null, "Ticket Type",
                "Amusement Park",
                JOptionPane.QUESTION_MESSAGE, null, choices, choices[0]);
        if (decision == null) {
            return "Cancel";
    ...
    public static String option5() {
        ...
        String decision = (String) JOptionPane.showInputDialog(null, "Ticket Class",
                "Amusement Park",
                JOptionPane.QUESTION_MESSAGE, null, choices, choices[0]);
        if (decision == null) {
            return "Cancel";
    ...
    public static String option8Warning() {
        int warning = JOptionPane.showConfirmDialog(null,
                "Warning: Are you sure you would like to quit?",
