...
public final class CountdownRequestHandler implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            try {
                answer = getTimeRemaining(message);
                writer.write(answer);
                writer.newLine();
                writer.flush();
                writer.close();
                ...
                BufferedWriter writer1 = new
                        BufferedWriter(new OutputStreamWriter(this.clientSocket.getOutputStream()));
                writer1.write("Error: the specified event date is incorrectly formatted!");
                writer1.newLine();
                writer1.flush();
                writer1.close();
            }
